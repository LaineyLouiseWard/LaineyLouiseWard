...
\begin{columns}[T]
    \begin{column}{0.60\textwidth}
        ...
        {\large\bfseries\color{darkink}
         Abrupt \textit{\textcolor{drybrown}{Drought Termination}} in the
    ...
    \begin{column}{0.40\textwidth}
        \vspace{0.5cm}
        \includegraphics[width=4.5cm, height=6.4cm]{drought_bg.png}
    \end{column}
\end{columns}
...
\begin{columns}[c]
    ...
    \begin{column}{0.52\textwidth}
        ...
        {\normalsize\bfseries\color{darkink} \textcolor{naored}{NAO+} pushes jet stream south}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[font=\bfseries\footnotesize, wetblue] at (0,2.6) {IVT (ERA-20C)};

        % Arrows between source boxes
        \draw[-{Stealth[length=3mm]}, thick, ucdgreen!60] (s1) -- (s2);

        % Sub-slide 2: Binary series box + arrows to it
        \onslide<2>{
...
\begin{columns}[c]
    \begin{column}{0.40\textwidth}
        ...
        \raisebox{-0.4em}{\includegraphics[height=1.4em,keepaspectratio]{ireland_icon.png}}\;
        {\normalsize\bfseries\color{darkink} no seasonality}
        }
    ...
    \begin{column}{0.60\textwidth}
        ...
        \begin{center}
            \begin{tikzpicture}
                \node[inner sep=0pt] (plot) {\includegraphics[width=0.82\textwidth]{fig4c_only.jpg}};
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[font=\Large\bfseries, darkink, anchor=west] at (-3.8,2.0)
            {\color{wetblue}\textit{Atmospheric rivers} \color{darkink}drive drought termination};
        ...
        \node[font=\Large\bfseries, darkink, anchor=west] at (-3.8,0.0)
            {\color{ucdgreen}\textit{Abruptness, seasonality, region}};

        % Line 3
        \draw[gray!30, thick] (-5,-1.0) -- (5,-1.0);
        \node[font=\footnotesize, gray] at (-4.5,-2.0) {03};
        \node[font=\Large\bfseries, darkink, anchor=west] at (-3.8,-2.0)
            {\color{naored}\textit{Untapped potential}};
