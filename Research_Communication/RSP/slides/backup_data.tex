...
\begin{frame}{Three strategies for detecting transitions}
    ...
    \begin{tikzpicture}[xshift=0.5cm,
        box/.style={draw=#1!70, thick, rounded corners=3pt,
                    fill=#1!8, minimum width=3.0cm, minimum height=0.7cm,
                    align=center, font=\scriptsize},
        data/.style={draw=gray!50, thick, rounded corners=3pt,
                    fill=gray!6, minimum width=3.2cm, minimum height=0.7cm,
                    align=center, font=\scriptsize},
        result/.style={draw=#1!70, thick, rounded corners=3pt,
                    fill=#1!15, minimum width=3.2cm, minimum height=0.7cm,
                    align=center, font=\scriptsize\bfseries},
        arr/.style={-{Stealth[length=2mm]}, thick, #1!60},
        lbl/.style={font=\footnotesize\bfseries, text=#1},
    ]

    % === Column 1: Raw thresholding ===
    \node[lbl=strat1] (t1) at (1.0, 3.8) {1. Raw thresholding};
    \node[data] (d1) at (1.0, 3.0) {Streamflow};
    \node[box=strat1] (th1) at (1.0, 1.8) {Threshold directly\\(e.g.\ $<$Q20, $>$Q90)};
    \node[box=strat1] (link1) at (1.0, 0.6) {Link drought + flood\\episodes by proximity};
    \node[result=strat1] (ev1) at (1.0, -0.6) {DTF events};

    \draw[arr=strat1] (d1) -- (th1);
    \draw[arr=strat1] (th1) -- (link1);
    \draw[arr=strat1] (link1) -- (ev1);

    \node[font=\tiny, text=gray, anchor=north, text width=3cm, align=center]
        at (1.0, -1.15) {Anderson, G\"otte \& Brunner,\\Yang, Matan\'o};

    % === Column 2: Index-based thresholding ===
    \node[lbl=strat2] (t2) at (5.5, 3.8) {2. Index-based};
    \node[data] (d2) at (5.5, 3.0) {Precipitation};
    \node[box=strat2] (idx) at (5.5, 1.8) {Compute index\\(SPI, SPEI, LDFAI, etc.)};
    \node[box=strat2] (th2) at (5.5, 0.6) {Threshold the index\\to identify events};
    \node[result=strat2] (ev2) at (5.5, -0.6) {DTF events};

    \draw[arr=strat2] (d2) -- (idx);
    \draw[arr=strat2] (idx) -- (th2);
    \draw[arr=strat2] (th2) -- (ev2);

    \node[font=\tiny, text=gray, anchor=north, text width=3cm, align=center]
        at (5.5, -1.15) {Wang, Bai, Zhao};

    % === Column 3: Regime classification ===
    \node[lbl=strat3] (t3) at (10.5, 3.8) {3. Regime classification};
    \node[data] (d3) at (10.5, 3.0) {500\,hPa height\\anomalies};
    \node[box=strat3] (som) at (10.5, 1.8) {Cluster circulation\\patterns};
    \node[box=strat3] (shift) at (10.5, 0.6) {Identify regime\\shifts};
    \node[result=strat3] (ev3) at (10.5, -0.6) {DTF events};

    \draw[arr=strat3] (d3) -- (som);
    \draw[arr=strat3] (som) -- (shift);
    \draw[arr=strat3] (shift) -- (ev3);
    ...
    \node[draw=gapred!40, rounded corners=3pt, fill=gapred!4,
          text width=12cm, align=left, font=\scriptsize, inner sep=6pt]
        at (5.25, -2.6) {
            Yang et al.\ (2025): raw streamflow vs SPI yield \textbf{opposite frequency trends}
            over 7 decades on the same US catchments.
        };
...
\begin{frame}{Verification Metrics}
    ...
    \begin{tikzpicture}
        \node[draw=ucdgreen!30, rounded corners=3pt, fill=ucdgreen!4,
              text width=12.5cm, align=left, font=\scriptsize, inner sep=6pt] {
            Bias and RMSE computed on \textbf{raw values}; ACC on \textbf{anomalies}.
            All aggregated to \textbf{weekly means} before scoring.
            Climatology: 1991--2020 (WMO standard).
        };
    \end{tikzpicture}
